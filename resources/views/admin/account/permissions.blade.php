...
<div class="content-wrapper">
    <div class="page-header d-flex justify-content-between align-items-center">
        <h3 class="page-title"><i class="fas fa-laptop"></i>Quản lý quyền</h3>
    </div>
    <div class="row p-2">
        <div class="col-xs-12 col-md-3">
            <form data-action="{{route('permissions.store')}}" class="g-3 needs-validation ajax-form-post" method="post"
                novalidate>
                @csrf
                <div class="mb-3">
                    <label for="permissionName" class="form-label">Tên quyền</label>
                    <input type="text" class="form-control" name="in_name" id="permissionName" placeholder="Tên quyền" required>
                    ...
                    <table class="table table-hover" id="tblPermission" class="display" style="width:100%">
                        ...
                        <tbody class="font-size-1">
                            ...
                            <tr class="replaywith-{{$value->id}}">
                                ...
                                <td>{{$value->name}}</td>
                                <td>
                                    <button type="button" class="btn btn-warning ajax-edit" data-id="{{$value->id}}"
                                        data-name="{{$value->name}}"><i class="fas fa-edit"></i></button>
                                    <button type="button" class="btn btn-danger ajax-delete"
                                        data-url="{{route('permissions.destroy', $value->id)}}"><i
...
<div class="offcanvas offcanvas-end" id="offcanvas_edit">
    ...
    <div class="offcanvas-body">
        <form data-action="{{route('permissions.update', 1)}}" data-element="#tblroles tbody"
