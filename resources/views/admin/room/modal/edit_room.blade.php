    ...
    <div class="modal-dialog modal-dialog-centered modal-lg" role="document">
        <div class="modal-content">
            <div class="modal-header">
                <h5 class="modal-title" id="form-title">Sửa phòng</h5>
                <button type="button" class="close" data-bs-dismiss="modal" aria-label="Close">
                    <span aria-hidden="true">&times;</span>
                </button>
            </div>
            <form id="edit-room-form" action="{{route('phong.update', $room->id)}}" method="post">
                    ...
                    <div class="tab-content" id="myTabContent">
                            ...
                            <div class="row">
                                <div class="col-12 col-sm-6">
                                    <div class="form-group">
                                        ...
                                        <input type="text" name="code" value="{{$room->code}}" class="form-control" placeholder="Mã phòng"
                                            required>
                                    </div>
                                    <div class="form-group">
                                        <label for="">Loại <sup class="text-danger">*</sup></label>
                                        <select name="type" class="form-control" required>
                                            <option value="1" {{$room->type == 1 ? ' selected' : ''}}>Phòng trọ</option>
                                            <option value="2" {{$room->type == 2 ? ' selected' : ''}}>Căn hộ</option>
                                            <option value="3" {{$room->type == 3 ? ' selected' : ''}}>Chung cư mini</option>
                                        </select>
                                    </div>
                                    <div class="form-group">
                                ...
                                <div class="col-12 col-sm-6">
                                    <div class="form-group">
                                        ...
                                        <input type="text" class="form-control" name="name" value="{{$room->name}}" placeholder="Tên phòng"
                                            required>
                                    </div>
                                    <div class="form-group">
                                        <label for="">Tầng <sup class="text-danger">*</sup></label>
                                        <select name="floor_id" class="form-control" required>
                                            @foreach ($room->building->floor()->get() as $floor)
                                                <option value="{{ $floor->id }}"
                                                    {{ $room->building->floor_id == $floor->id ? ' selected' : '' }}>
                                                    {{ $floor->name }}
                                                </option>
                                            @endforeach
                                        </select>
                                    ...
                                    <div class="form-group">
                                        ...
                                        <input type="number" class="form-control" name="acreage" value="{{$room->acreage}}"
                                ...
                                <div class="col-12">
                                    <div class="form-group">
                                        ...
                                        <textarea name="note" id="" rows="10" class="w-100">{{$room->note}}</textarea>
                                    </div>
                                </div>
                            </div>
                        </div>
                            ...
                            <div class="form-group">
                                ...
                                <div class="form-group">
                                    ...
                                    <img id="avatar" class="add-image-ckfinder pointer" data-preview="#avatar"
                                        ...
                                        src="{{ asset($room->avatar) }}" alt="" style="width: 100%">
                                </div>
                            </div>
                        </div>
                        <div class="tab-pane fade show" id="asset" role="tabpanel">
                            <div class="form-group">
                                ...
                                <textarea class="editor" name="asset">{{$room->asset}}</textarea>
                            </div>
                        </div>
                        <div class="tab-pane fade show" id="extend" role="tabpanel">
                            <div class="form-group">
                                <label for="">Tên Hiển thị blog <sup class="text-danger">*</sup></label>
                                <input type="text" class="form-control" name="name_blog" value="{{$room->name_blog}}"
                                    placeholder="Tên hiển thị bài viết khách hàng">
                            </div>
                            <div class="form-group">
                                <label for="">Mô tả</label>
                                <textarea class="editor" name="description">{{$room->description}}</textarea>
                            </div>
                        </div>
                    </div>
